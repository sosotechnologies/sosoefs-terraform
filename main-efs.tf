...
data "aws_iam_policy_document" "policy" {
  ...
  dynamic "statement" {
    ...
    content {
      ...
      resources     = try(statement.value.resources, [aws_efs_file_system.sosotech[0].arn], null)
  ...
  statement {
    ...
    effect    = "Deny"
    actions   = ["*"]
    resources = [aws_efs_file_system.sosotech[0].arn]
...
resource "aws_efs_file_system_policy" "sosotech" {
  count = var.create && var.attach_policy ? 1 : 0

  file_system_id                     = aws_efs_file_system.sosotech[0].id
...
resource "aws_efs_mount_target" "sosotech" {
  ...
  security_groups = var.create_security_group ? concat([aws_security_group.sosotech[0].id], try(each.value.security_groups, [])) : try(each.value.security_groups, null)
...
resource "aws_security_group" "sosotech" {
...
resource "aws_security_group_rule" "sosotech" {
  for_each = { for k, v in var.security_group_rules : k => v if var.create && var.create_security_group }

  security_group_id = aws_security_group.sosotech[0].id
...
resource "aws_efs_replication_configuration" "sosotech" {
  ...
  source_file_system_id = aws_efs_file_system.sosotech[0].id
